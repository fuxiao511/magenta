...
mx_status_t sys_bti_pin(mx_handle_t bti, mx_handle_t vmo, uint64_t offset, uint64_t size,
    ...
    const size_t max_len = size / PAGE_SIZE;
    if (extents_len < max_len) {
    ...
    AllocChecker ac;
    mxtl::InlineArray<dev_vaddr_t, 4u> mapped_extents(&ac, max_len);
    if (!ac.check()) {
        return MX_ERR_NO_MEMORY;
    }
    ...
    status = bti_dispatcher->Pin(vmo_dispatcher->vmo(), offset, size, iommu_perms,
                                 mapped_extents.get(), max_len, &actual_len);
    if (status != MX_OK) {
        return status;
    }
